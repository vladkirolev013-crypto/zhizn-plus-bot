...
# ТВОИ ПРАВИЛЬНЫЕ КЛЮЧИ (БЕЗ ДВОЙНОГО КОДИРОВАНИЯ)
...
# ============================================
# УБИВАЕМ 409 НАВСЕГДА (УДАЛЕНИЕ OFFSET-ФАЙЛОВ)
# ============================================
def kill_409_forever():
    """Полностью убивает 409: удаляет offset-файлы и вебхук"""
    try:
        # 1. Удаляем offset-файлы (ГЛАВНАЯ ПРИЧИНА 409)
        offset_files = glob.glob('update-offset-*.json')
        for f in offset_files:
            try:
                os.remove(f)
                logger.info(f"🧹 Удалён offset-файл: {f}")
            except:
                pass
        
        # 2. Удаляем вебхук через API
        url = f"https://api.telegram.org/bot{BOT_TOKEN}/deleteWebhook"
        response = requests.post(url, json={"drop_pending_updates": True})
        logger.info(f"🧹 Удаление вебхука: {response.text}")
        
        # 3. Проверяем что вебхук удалён
        time.sleep(1)
        check = requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/getWebhookInfo")
...
def ask_giga(system, user, max_tokens=2500):
    """Запрос к GigaChat (ТОЛЬКО ЧЕРЕЗ GIGACHAT)"""
...
def ask_giga_with_wait(system, user, max_tokens=2500):
    """Ждёт 30 секунд (ДАЖЕ ЕСЛИ GIGACHAT ОТВЕТИЛ БЫСТРО)"""
    start_time = time.time()
    result = ask_giga(system, user, max_tokens)
    elapsed = time.time() - start_time
    
    # Если GigaChat ответил быстрее 30 секунд — ждём
    if elapsed < 30:
...
# ============================================
TEST_TOPICS = {
    "психология": "🧠 Психологическое состояние, эмоции, стресс",
...
# ============================================
# НЛП-ПРОМПТЫ (БЕЗ СЛАЩАВОСТИ)
# ============================================
def generate_test_questions(topic, count=10):
    system = """Ты — клинический психолог с 25-летним стажем.
    Ты не задаёшь шаблонных вопросов. Твои вопросы заставляют человека задуматься.
    Вопросы должны быть живыми, конкретными, создавать образы и чувства.
    Без штампов: «Как вы оцениваете...», «Как вы относитесь...»
    Вопросы — как маленькие встречи с собой.
    Каждый вопрос уникален."""
...
def generate_analysis_free(topic, answers, score, total):
    ...
    Ты не утешаешь, ты даёшь опору.
    Ты используешь НЛП-язык: пресуппозиции, присоединение к реальности.
    Ты не даёшь советов, ты задаёшь вопросы.
    Без воды, без шаблонов.
    Объём: 700+ знаков. Только разбор + 2 вопроса для размышления."""
...
1. КЛИНИЧЕСКИЙ ПСИХОЛОГ:
   ...
   - Ты используешь пресуппозиции: «Когда ты осознаешь этот паттерн...»
...
2. КОУЧ МИРОВОГО УРОВНЯ:
   ...
   - Ты даёшь 3 конкретных шага, которые можно сделать сегодня.
...
Включи: глубокий психологический портрет, 2 инсайта, конкретные шаги на неделю, книги, упражнения, видео."""
...
def generate_post():
    ...
    Ты используешь НЛП-язык: пресуппозиции, присоединение.
    Без слащавости. Каждый пост уникальный."""
