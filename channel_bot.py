...
# ⚠️ ВАЖНО: Authorization key (уже в Base64) - НЕ КОДИРУЕМ!
...
# ============================================
# GIGACHAT (ИСПРАВЛЕННАЯ ВЕРСИЯ)
# ============================================

giga_token_cache = {"token": None, "expires": 0}

def get_giga_token():
    """Получение токена GigaChat — используем готовый Authorization key"""
    
    if giga_token_cache["token"] and time.time() < giga_token_cache["expires"]:
        return giga_token_cache["token"]
    
    try:
        # ⚠️ ВАЖНО: Используем AUTH_KEY КАК ЕСТЬ (уже в Base64)
        # НЕ кодируем повторно!
        auth_b64 = AUTH_KEY
